...
14. **Live citation health** — On thorough and deep, every cited URL is probed server-side and classified `live` (resolves now), `stale` (link rot — archived once), or `dead` (no DNS, no archive — likely fabricated). Deep-research agents hallucinate a meaningful share of citation URLs; this makes "every citation resolves" a checkable guarantee.
