...
- **Live citation health:** On thorough/deep (premium), every cited URL is probed server-side (`apps/api/src/lib/urlHealth.ts` in the engine repo): HEAD/GET liveness with a Wayback fallback classifies each source as `live`, `stale` (link rot — archived once), or `dead` (no DNS + no archive — likely fabricated). Sets `urlHealth` per source and a `citationHealth` summary on the result. Bounded concurrency, tight timeouts, fails open. Addresses the finding that deep-research agents hallucinate 3–13% of citation URLs — and no competitor verifies this.
